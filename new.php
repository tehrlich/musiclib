...
if($priv == 'Director' or $priv == 'Librarian'):
	$sql = "SELECT ID FROM  `Works` WHERE ID <>  'brass' AND ID <>  'ww' AND ID <>  'sax' ORDER BY CAST( ID AS UNSIGNED ) DESC LIMIT 1";
	$result = mysql_query($sql) or die(mysql_error()); 
	$nextID = mysql_fetch_array($result)[0]+1;
...
<form action='' method='POST'> 
	<p><b>ID:</b><br /><input type='text' name='ID' value="<?php echo($nextID);?>"/> 
